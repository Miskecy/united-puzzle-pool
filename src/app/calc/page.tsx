...
// Componentes UI assumidos (Button, Input, Tooltip, Alert, Accordion)
...
export default function ToolPage() {
	...
	// --- Funções de Lógica (Necessárias para o cálculo local) ---
	...
	// Formata o BigInt com casas decimais e precisão
	function formatDiv(n: bigint, unit: bigint, decimals: number = 2): string {
		const whole = n / unit;
		const rem = n % unit;
		if (decimals <= 0) return whole.toString();

		const scale = BigInt(10 ** decimals);
		const frac = (rem * scale) / unit;
		const fracStr = frac.toString().padStart(decimals, '0');

		return `${whole.toString()}.${fracStr}`;
	}

	// Função de Operação Principal (execução local)
	async function runOp(op: string) {
		setLoading(true);
		setError(null);

		// Verifica se a operação pode ser executada localmente (pow_to_decimal, decimal_to_pow, pow_to_keys)
		...
		try {
			if (isLocalOp) {
				...
				for (let i = 0; i < lines.length; i++) {
					...
					if (op === 'pow_to_decimal' || op === 'pow_to_keys' || op === 'pow_to_keys_all') {
						...
						if (!m) {
							outputs.push('');
							errs.push('Expected format 2^<int>');
							continue;
						}
						const exp = BigInt(m[1]);
						if (exp < 0 || exp > 256) {
							outputs.push('');
							errs.push('Exponent must be between 0 and 256');
							continue;
						}
						const val = 1n << exp;

						if (op === 'pow_to_decimal') {
							outputs.push(val.toString());
							errs.push(null);
						} else if (op === 'pow_to_keys') {
							// B/T/P Keys
							const B = 1000000000n;
							const T = 1000000000000n;
							const P = 1000000000000000n;
							const b = formatDiv(val, B, precision);
							const t = formatDiv(val, T, precision);
							const p = formatDiv(val, P, precision);
							outputs.push(`BKeys ${b} | TKeys ${t} | PKeys ${p}`);
							errs.push(null);
						} else {
							// K/M/G/B/T/P/E Keys
							const K = 1000n;
							...
							const T = 1000000000000n;
							const P = 1000000000000000n;
							const E = 1000000000000000000n;

							const k = formatDiv(val, K, precision);
							const m2 = formatDiv(val, M, precision);
							const g = formatDiv(val, G, precision);
							const t2 = formatDiv(val, T, precision);
							const p2 = formatDiv(val, P, precision);
							const e = formatDiv(val, E, precision);
							outputs.push(`KKeys ${k} | MKeys ${m2} | GKeys ${g} | TKeys ${t2} | PKeys ${p2} | EKeys ${e}`);
							errs.push(null);
						}
						...
						if (!/^\d+$/.test(s)) {
							outputs.push('');
							errs.push('Expected positive integer decimal');
							continue;
						}
						const n = BigInt(s);
						if (n <= 0n) {
							outputs.push('');
							errs.push('Value must be > 0');
							continue;
						}
				...
				// Atualiza o input com o output
				setInput(outputs.join('\n'));
				const hasErr = errs.some((e) => !!e);

				if (hasErr) {
					const messages = errs
						.map((e, i) => (e ? `Line ${i + 1}: ${e}` : null))
						.filter(Boolean)
						.join('\n');
					setError(messages);
				} else {
			...
			// --- Operações de API (Priv/Pub Key, Hash, Base58, Encoding) ---
			const res = await fetch('/api/tool/operate', {
				method: 'POST',
			...
			if (!res.ok) {
				setError(data?.error || 'Failed to complete API operation');
			} else {
				const errs: (string | null)[] = data.errors || [];
				const hasErr = errs.some((e: string | null) => !!e);

				setInput((data.outputs || []).join('\n'));

				if (hasErr) {
					const messages = errs
						.map((e: string | null, i: number) => (e ? `Line ${i + 1}: ${e}` : null))
						.filter(Boolean)
						.join('\n');
					setError(messages);
				} else {
			...
			console.error(e);
			setError('Network or unexpected error during operation.');
		} finally {
			setLoading(false);
		}
	}

	// --- Componente OperationButton Padronizado ---
		...
		return (
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						// Aplica o estilo primário se for uma operação de Keyspace/Power
						variant={isPrimaryOp ? 'default' : 'outline'}
						size="sm"
						onClick={() => runOp(op)}
						disabled={loading}
						className={`font-medium shadow-sm w-full justify-start text-left text-sm whitespace-nowrap overflow-hidden transition-colors duration-200
                    ${isPrimaryOp ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border border-gray-300'}`}
					>
	...
	// Define as operações de precisão
		...
		<TooltipProvider delayDuration={200}>
			...
			<div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 py-12">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					{/* Layout Principal: 2 Colunas (Input/Output [FIXO] + Operações) */}
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">

						{/* Coluna 1: Input/Output (FIXO em altura) */}
						<Card className="lg:col-span-2 bg-white border-gray-200 shadow-md min-h-[700px] flex flex-col">
							...
							{/* Conteúdo flexível para o textarea ocupar o espaço restante */}
							<CardContent className="pt-6 flex flex-col flex-1">
								<textarea
									// Adicionado flex-1 e min-h para garantir que o textarea preencha o espaço fixo
									className="w-full border border-gray-300 rounded-lg px-4 py-4 text-gray-900 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-600 font-mono text-sm shadow-inner transition-shadow flex-1 min-h-[250px]"
									placeholder="Paste inputs, each on a new line (e.g., 2^250 or 0x1A2B...)"
									value={input}
								...
								<div className="flex justify-between items-center mt-4">
									<Button
										onClick={clearInput}
										variant="outline"
										className="bg-red-50 text-red-600 border-red-300 hover:bg-red-100 hover:text-red-700"
										disabled={loading}
									>
										Clear Input
									</Button>
									{error && (
										<Alert className="max-w-md bg-red-50 border-red-300 text-red-800">
						...
						{/* Coluna 2: Operações e Ferramentas (Agora mantém sua altura natural e o Input não estica) */}
							...
							<CardContent className="pt-6">
								<div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
									...
									<div className="flex items-center gap-2">
										<Input
											type="number"
											className="w-24 h-9 bg-white text-center font-mono border-gray-300"
											value={precision}
											onChange={(e) => {
								...
								<Accordion type="multiple" className="divide-y divide-gray-200">
									...
									<AccordionItem value="keys" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><Key className="h-4 w-4 text-purple-600" /> Keys (Private)</span>
										</AccordionTrigger>
									...
									<AccordionItem value="public" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><Key className="h-4 w-4 text-purple-600" /> Public Key</span>
										</AccordionTrigger>
										<AccordionContent className='pb-4'>
											<div className="flex flex-col gap-2">
									...
									<AccordionItem value="hashes" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><Hash className="h-4 w-4 text-purple-600" /> Hashes</span>
										</AccordionTrigger>
										<AccordionContent className='pb-4'>
											<div className="flex flex-col gap-2">
									...
									<AccordionItem value="base58" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><LinkIcon className="h-4 w-4 text-purple-600" /> Base58 / Base58Check</span>
										</AccordionTrigger>
										<AccordionContent className='pb-4'>
											<div className="flex flex-col gap-2">
									...
									<AccordionItem value="encoding" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><Text className="h-4 w-4 text-purple-600" /> Encoding</span>
										</AccordionTrigger>
										<AccordionContent className='pb-4'>
											<div className="flex flex-col gap-2">
									...
									<AccordionItem value="exponent" className='border-gray-200'>
										<AccordionTrigger className="text-gray-900 hover:no-underline">
											<span className="flex items-center gap-2 font-medium"><Sigma className="h-4 w-4 text-purple-600" /> Exponent / Keyspace</span>
										</AccordionTrigger>
										<AccordionContent className='pb-4'>
											<div className="flex flex-col gap-2">

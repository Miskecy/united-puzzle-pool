...
function SortButton({ label, keySel, sortKey, sortDir, onClick }: { label: string; keySel: SortKey; sortKey: SortKey; sortDir: 'asc' | 'desc'; onClick: () => void }) {
	...
	return (
		<Button
			...
			size="sm"
			onClick={onClick}
			// Strict blue/gray/white styling
			className={`${isActive ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md' : 'border-gray-300 text-gray-700 hover:bg-gray-100'} transition-all font-medium`}
		>
			{label}
			{isActive && (sortDir === 'desc' ? <ChevronDown className="ml-1 w-4 h-4" /> : <ChevronUp className="ml-1 w-4 h-4" />)}
		</Button>
	)
}
...
export default function GPURankingPage() {
	// --- State and Data Logic (No change) ---
	...
	const userAggregated = useMemo(() => {
		const byModel = new Map<string, { model: string; speeds: number[]; brand?: string; architecture?: string; series?: string; tdp_w?: number }>()
		for (const it of userItems.filter(i => i.status === 'APPROVED')) {
			const cur = byModel.get(it.model) || { model: it.model, speeds: [], brand: it.brand, architecture: it.architecture, series: it.series, tdp_w: it.tdp_w }
			cur.speeds.push(it.approx_keys_per_second_mkeys)
			byModel.set(it.model, cur)
		}
		const rows = Array.from(byModel.values()).map(v => {
			const avg = v.speeds.length ? (v.speeds.reduce((a, b) => a + b, 0) / v.speeds.length) : 0
			const spec = gpuSpecs.get(v.model)
			return {
				model: v.model,
				approx_keys_per_second_mkeys: avg,
				architecture: spec?.architecture ?? v.architecture ?? '-',
				series: spec?.series ?? v.series ?? '-',
				cuda_cores: spec?.cuda_cores ?? 0,
				tdp_w: spec?.tdp_w ?? v.tdp_w ?? 0,
				brand: spec?.brand ?? v.brand ?? '-',
			}
		})
		return rows.sort((a, b) => b.approx_keys_per_second_mkeys - a.approx_keys_per_second_mkeys)
	}, [userItems, gpuSpecs])

	const userSpeedMax = useMemo(() => {
		return userAggregated.length ? Math.max(...userAggregated.map(d => d.approx_keys_per_second_mkeys)) : 0
	}, [userAggregated])

	const userEffMax = useMemo(() => {
		return userAggregated.length ? Math.max(...userAggregated.map(d => computeEfficiency({ model: d.model, cuda_cores: d.cuda_cores, architecture: d.architecture, series: d.series, tdp_w: d.tdp_w ?? 0, approx_keys_per_second_mkeys: d.approx_keys_per_second_mkeys }))) : 0
	}, [userAggregated])
	...
	async function fetchUserGpus() {
		try {
			const r = await fetch('/api/user-gpus')
			if (r.ok) {
				const j = await r.json()
				setUserItems(Array.isArray(j.items) ? j.items : [])
			}
		} catch { }
	}
	...
	const architectures = useMemo(() => {
		...
		for (const d of data) set.add(d.architecture)
		return ['All', ...Array.from(set)]
	}, [data])

	const seriesList = useMemo(() => {
		const set = new Set<string>()
		for (const d of data) set.add(d.series)
		return ['All', ...Array.from(set)]
	}, [data])

	const models = useMemo(() => {
		const set = new Set<string>()
		for (const d of data) set.add(d.model)
	...
	const filtered = useMemo(() => {
		...
		return data.filter(d => {
			if (arch !== 'All' && d.architecture !== arch) return false
		...
		arr.sort((a, b) => {
			...
			switch (sortKey) {
				case 'rank':
					...
					break
				case 'speed':
					av = a.approx_keys_per_second_mkeys
					bv = b.approx_keys_per_second_mkeys
					break
				case 'efficiency':
					av = computeEfficiency(a)
					bv = computeEfficiency(b)
					break
				case 'cuda':
					av = a.cuda_cores
					bv = b.cuda_cores
	...
	const speedMax = useMemo(() => {
		return filtered.length ? Math.max(...filtered.map(d => d.approx_keys_per_second_mkeys)) : 0
	}, [filtered])
	const effMax = useMemo(() => {
		return filtered.length ? Math.max(...filtered.map(d => computeEfficiency(d))) : 0
	}, [filtered])
	...
	const avgSpeed = useMemo(() => {
		...
		const sum = sorted.reduce((acc, it) => acc + it.approx_keys_per_second_mkeys, 0)
	...
	// Using blue/gray/green (for positive) and red/gray (for negative)
	const betterClass = (cond: boolean, neutralColor: string = 'text-gray-900') => cond ? 'text-blue-600' : neutralColor

	function pickBest(a?: GPUItem, b?: GPUItem) {
		if (!a && !b) return null
		if (a && !b) return { best: 'A', item: a, reasons: ['Higher speed', 'Better efficiency'] }
		...
		if (item.approx_keys_per_second_mkeys >= (best === 'A' ? b.approx_keys_per_second_mkeys : a.approx_keys_per_second_mkeys)) reasons.push('Higher Speed')
		if (computeEfficiency(item) >= (best === 'A' ? eb : ea)) reasons.push('Better Efficiency')
		if (item.tdp_w <= (best === 'A' ? b.tdp_w : a.tdp_w)) reasons.push('Lower TDP')
		return { best, item, reasons }
	}
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
				...
				<Tabs defaultValue="official" className="w-full">
					<TabsList className="grid w-full grid-cols-2 h-auto p-1 mb-6 bg-white shadow-md border border-gray-200">
						<TabsTrigger value="official" className="text-sm py-2 px-3 data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-lg">GPU Ranking (AI Generated)</TabsTrigger>
					...
					<TabsContent value="official" className="space-y-6">
						...
						{/* --- GPU COMPARISON CARD (Minimalist Style) --- */}
								...
								{pickBest(itemA, itemB) && (
									<div className="mt-6 rounded-lg border-2 border-blue-400 bg-blue-50/50 p-4 shadow-md">
										...
										<div className="mt-2 flex items-center gap-3 text-base text-gray-900 font-semibold">
											<span className="text-blue-600">{pickBest(itemA, itemB)?.item.model}</span>
										</div>
										<div className="mt-3 flex flex-wrap gap-2 text-sm">
											{pickBest(itemA, itemB)?.reasons.map(r => (
												<Badge key={r} className="px-3 py-1 bg-white border border-blue-300 text-blue-600 font-medium">{r}</Badge>
											))}
										</div>
									</div>
								)}
						...
						{/* --- FILTERS & SORTING CARD (Clean Layout) --- */}
						<Card className="mb-10 shadow-lg border border-gray-200 bg-white">
							<CardHeader className="border-b px-6 py-4 ">
						...
						{/* --- GPU TABLE CARD (Professional Look) --- */}
						...
						{/* --- TOP 10 CHART CARD (Solid Blue Bars) --- */}
					...
					<TabsContent value="user" className="space-y-6">
										...
										<Button onClick={async () => {
											...
											try {
												...
												if (r.ok) {
													...
													setUserMsg('Failed to submit')
												}
											} catch { setUserMsg('Failed to submit') }
											finally { setUserLoading(false) }
										}} disabled={userLoading} className="w-full md:w-auto">
											Submit
										</Button>
									</div>
								</div>
								{userMsg && <div className="mt-3 text-sm text-gray-700">{userMsg}</div>}
							</CardContent>
						</Card>
						...
						<Card className="shadow-lg border border-gray-200 bg-white">
							<CardHeader className="border-b px-6 py-4 ">
								...
								<CardDescription className="text-gray-600">Approved submissions aggregated by model and matched to official specs.</CardDescription>
							</CardHeader>
							<CardContent className="p-0">
								<div className="overflow-x-auto">
									<Table>

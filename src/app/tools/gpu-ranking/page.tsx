...
function fmtEff(val: number, unit: Unit) { const v = toEff(val, unit); if (unit === 'G' && v < 0.1) return NF_4DP.format(v); return (unit === 'G' ? NF_2DP : NF_1DP).format(v) }
...
export default function GPURankingPage() {
	...
	const gpuSpecs = useMemo(() => { type SpecRow = { Brand: string; Model: string; Architecture: string; Series: string; CoreUnits: number; TDP_W: number | null }; const rows: SpecRow[] = Array.isArray(specsData) ? (specsData as SpecRow[]) : []; const map = new Map<string, { brand: string; model: string; architecture: string; series: string; cuda_cores: number; tdp_w: number }>(); for (const s of rows) { map.set(s.Model, { brand: s.Brand, model: s.Model, architecture: s.Architecture, series: s.Series, cuda_cores: Number(s.CoreUnits || 0), tdp_w: Number.isFinite(Number(s.TDP_W)) ? Number(s.TDP_W) : 0, }) } return map }, [])
	...
	const data = useMemo(() => { const list = Array.from(gpuSpecs.values()).map(s => { const agg = userAggregated.find(u => u.model === s.model); const speed = agg && agg.approx_keys_per_second_mkeys > 0 ? agg.approx_keys_per_second_mkeys : estimateSpeedMKeys({ architecture: s.architecture, cuda_cores: s.cuda_cores }); return { model: s.model, cuda_cores: s.cuda_cores, architecture: s.architecture, series: s.series, tdp_w: s.tdp_w, approx_keys_per_second_mkeys: speed } }); const ranked = list.slice().sort((a, b) => b.approx_keys_per_second_mkeys - a.approx_keys_per_second_mkeys).map((it, idx) => ({ ...it, rank: idx + 1 })); return ranked }, [gpuSpecs, userAggregated])
